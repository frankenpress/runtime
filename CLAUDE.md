...
- `web/index.php` — tiny test app emitting `Surrogate-Key` headers + a microsecond timestamp. Not the production WordPress entry point — that comes from `fp-site-template`. **Don't delete: `tests/cache-spike.sh` depends on it.**
...
- **fp-mu-plugin is baked at `/app/web/app/mu-plugins/fp/`** by default. The bake step downloads the tarball for `FP_MU_PLUGIN_VERSION` (defaults to a specific tag, e.g. `v0.1.1`). Pass an empty string to skip baking when the consumer site composer-installs it themselves.
...
- **Don't change the cache key shape (`GET-<scheme>-<host>-<path>`)** without coordinating with `fp-mu-plugin`'s `SouinInvalidator`. The mu-plugin DELs Redis keys directly using this exact prefix; a change here breaks invalidation silently.
...
make build          # just rebuild fp-runtime:dev
